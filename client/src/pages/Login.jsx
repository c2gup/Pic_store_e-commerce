...
import { useDispatch } from "react-redux";
import { login } from "../../store/slices/authSlice";
import googleAuth from "../../utils/firebase"; // Import googleAuth utility
...
export default function SignInWithGoogle() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      setLoading(true);
      const data = await googleAuth();  // Assume this returns the necessary token from Google
      
      if (data?.accessToken) {
        // Send token to backend for verification and authentication
        handleLogin({ googleToken: data.accessToken });
      } else {
  ...
  // Handle login by sending data to the backend
  const handleLogin = (data) => {
    fetch('https://pic-prism-my-jngk.vercel.app/api/login', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      credentials: 'include', // Use 'include' to send cookies
      body: JSON.stringify(data), // Send the Google token
    })
      ...
      .then(result => {
        ...
        if (result.success) {
          ...
          dispatch(login(result.data)); // Assuming result contains user data
          navigate(`/${result.data.role}/profile`);
  ...
  return (
    <div>
      <button
        onClick={handleGoogleSignIn}
        disabled={loading}
        className="bg-white text-black py-2 px-6 rounded-full flex items-center"
      >
        <img
          src="https://upload.wikimedia.org/wikipedia/commons/a/a0/Google_2015_logo.svg"
          alt="Google"
